...
export const env = createEnv({
  ...
  // return the env shape, when skipping validation.
  getUnvalidatedEnv: (env, schema) => env
});
```

For type safety, you're required to provide a `getUnvalidatedEnv` callback if using `skipValidation`. This provides the opportunity to extract defaults from your schema, or to apply any necessary transformations the schema would normally be in charge of.
