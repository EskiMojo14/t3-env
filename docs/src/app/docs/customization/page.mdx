...
For better type safety, you're able to provide a `getUnvalidatedEnv` callback. This provides the opportunity to extract defaults from your schema, or to apply any necessary transformations the schema would normally be in charge of.
...
export const env = createEnv({
  ...
  // ...
  getFinalSchema: (shape) => v.object(shape),
  skipValidation: true,
